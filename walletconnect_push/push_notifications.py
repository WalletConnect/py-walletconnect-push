...
class PushNotificationsService(object):
  ...
  def __init__(self, session=None, api_key=None, debug=False):
  ...
  async def notify_single_device(self, registration_id, message_body, data_message):
    if self.debug:
      print(data_message)
      return True
    payload = self.generate_payload(registration_id, message_body, data_message)
    response = await self.notify(payload)
    await self.parse_response(response)


  def generate_payload(self, registration_id, message_body, data_message):
    fcm_payload = {}
